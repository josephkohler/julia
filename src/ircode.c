...
static size_t codelocs_parseheader(jl_string_t *cl, int *line_offset, int *line_bytes, int *to_bytes) JL_NOTSAFEPOINT
{
    ...
    *line_offset = header[0];
    ...
    if (header[2] == 0)
        ...
        *to_bytes = 4;
    return (jl_string_len(cl) - sizeof(header)) / (*line_bytes + *to_bytes * 2); // compute nstmts
}
...
{
    const char *ptr = jl_string_data(cl) + sizeof(int32_t[3]) + pc * (line_bytes + to_bytes * 2);
    uint8_t int8;
    uint16_t int16;
    uint32_t int32;
...
JL_DLLEXPORT jl_string_t *jl_compress_codelocs(jl_value_t *codelocs, size_t nstmts) // Vector{Int32} => Memory{UInt8}
{
    ...
    size_t i;
    min.line = min.to = min.pc = INT32_MAX;
...
#undef SETMAX
    ...
    if (header[2] == 0)
        ...
        to_bytes = 4;
    jl_string_t *cl = jl_alloc_string(nstmts * (line_bytes + to_bytes * 2) + sizeof(header));
...
JL_DLLEXPORT jl_value_t *jl_uncompress_codelocs(jl_string_t *cl, size_t nstmts) // Memory{UInt8} => Vector{Int32}
{
    ...
    size_t i;
    for (i = 0; i < nlocs; i++) {
        struct jl_codeloc_t codeloc = unpack_codeloc(cl, i, line_offset, line_bytes, to_bytes);;
